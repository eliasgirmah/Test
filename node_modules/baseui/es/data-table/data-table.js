...
import { VariableSizeGrid } from 'react-window'; // @ts-ignore
...
function Header(props) {
  ...
  const isResizing = props.resizeIndex >= 0; // @ts-ignore
...
InnerTableElement.displayName = 'InnerTableElement'; // @ts-ignore
...
export function DataTable({
  ...
  const sortedIndices = React.useMemo(() => {
    ...
    if (index !== null && index !== undefined && index !== -1 && columns[index]) {
      const sortFn = columns[index].sortFn; // @ts-ignore
  ...
  const filteredIndices = React.useMemo(() => {
    const set = new Set(allRows.map((_, idx) => idx)); // @ts-ignore
  ...
  const [rowHighlightIndex, setRowHighlightIndex] = React.useState(-1); // @ts-ignore
  ...
  }), /*#__PURE__*/React.createElement(MeasureScrollbarWidth // @ts-ignore
  , {
